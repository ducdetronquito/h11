...
pub const ChunkedReader = struct {
    ...
    pub inline fn read(self: *Self, reader: anytype, buffer: []u8) !Event {
        while (true) {
    ...
    inline fn readChunkSize(self: *Self, reader: anytype, buffer: []u8) !?Event {
        var line = try readLine(reader, buffer[self.bytes_read..]);
    ...
    inline fn readChunk(self: *Self, reader: anytype, buffer: []u8) !?Event {
    ...
    inline fn readChunkEnd(self: *Self, reader: anytype) !?Event {
...
test "ChunkedReader - Ignore chunk extensions" {
    const content = "E;hash=toto\r\nGotta go fast!\r\n0\r\n\r\n";
    var reader = std.io.fixedBufferStream(content).reader();
